...
export default function TransactionsScreen() {
  ...
  const [periodic, setPeriodic] = useState([
    ...
    },
    {
      ...
      amount: "15",
      description: "",
      nextDueDate: new Date(new Date().setDate(new Date().getDate() + 1)),
    },
    {
      id: "3",
      category: "5",
    ...
    },
    {
      ...
      category: "7",
      icon: "📰",
      title: "NYT Subscription",
      times: "1",
      numOfPeriods: "4",
      frequency: "week(s)",
      amount: "-25",
      description: "",
      nextDueDate: new Date(new Date().setDate(new Date().getDate() + 28)),
    },
  ]);
  ...
  const [isNewTransactionMenuOpen, setIsNewTransactionMenuOpen] =
    useState(false);
  ...
  const handleSearch = async () => {
    const data: SingularTransactionEntry[] = await searchTransactions(
      selectedColumn,
      searchValue
    );
    setResults(data);
  ...
  const deleteSingular = () => {
    setSingular((prevSingular) =>
      prevSingular.filter((item) => item.id !== singularID)
    );
  };
  ...
  const renderSingularItem = ({ item }: { item: SingularTransactionEntry }) => (
    <TouchableOpacity
      style={styles.singularItem}
      onPress={() => showSingularEditModal(item)}
    >
      {/* <List.Icon icon="cash" color="black" /> */}
      <Text style={{ fontSize: 30 }}>{item.icon}</Text>
      <View style={styles.textContainer}>
        <Text style={styles.singularTitle}>{item.title}</Text>
      </View>
      <View style={[styles.transactionInfoContainer, { width: 80 }]}>
        <Text style={styles.singularDate}>{item.date}</Text>
      </View>
      <View
        style={[
          styles.transactionInfoContainer,
          { width: 60, paddingLeft: 15 },
        ]}
      >
        <Text
          style={[
            styles.singularAmount,
            {
              color:
                item.amount && String(item.amount).startsWith("-")
                  ? "#FF0000"
                  : "#00FF11",
            },
          ]}
  ...
  const createPeriodicRow = () => {
    return {
      ...
      nextDueDate: determineNextDueDate(
        periodicPeriod + "",
        periodicNumOfPeriods,
        new Date()
      ),
    };
  };
  ...
  const deletePeriodic = () => {
    setPeriodic((prevPeriodic) =>
      prevPeriodic.filter((item) => item.id !== periodicID)
    );
  };
  ...
  function processPeriodicTransactions(
    transactions: PeriodicTransactionEntry[]
  ) {
    const now = new Date();
    ...
    transactions.forEach((transaction) => {
      ...
      if (now >= nextDue) {
        ...
        let trackerEntry = idTracker.find(
          (entry) => entry.date === nextDueString
        );
        if (!trackerEntry) {
        ...
        transaction.nextDueDate = determineNextDueDate(
          transaction.frequency,
          transaction.numOfPeriods,
          nextDue
        );
      }
    });
  ...
  const renderPeriodicItem = ({ item }: { item: PeriodicTransactionEntry }) => (
    <TouchableOpacity
      style={styles.periodicItem}
      onPress={() => showPeriodicEditModal(item)}
    >
      {/* <List.Icon icon="cash" color="black" /> */}
      <Text style={{ fontSize: 50 }}>{item.icon}</Text>
      <Text style={[styles.periodicTitle]}>{item.title}</Text>
      <Text
        style={[
          styles.periodicAmount,
          {
            color:
              item.amount && String(item.amount).startsWith("-")
                ? "#FF0000"
      ...
      <Text style={styles.periodicFrequency}>
        {item.times}x/{item.numOfPeriods} {item.frequency}
      </Text>
      {/* <IconButton icon="dots-horizontal" /> */}
    </TouchableOpacity>
  );
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ marginLeft: 10 }}>
        ...
        >
          ...
          <Text style={[styles.pageHeader, { fontWeight: "bold" }]}>
            ${Balance()}
          </Text>
        </View>
        ...
        {/* Search Button */}
        <Button style={{ backgroundColor: "white" }} onPress={handleSearch}>
          Search
        </Button>
      ...
      <TouchableOpacity
        ...
        onPress={toggleNewTransactionMenu}
      >
        ...
        <>
          <View style={[styles.smallButtonContainer, { bottom: 90 }]}>
            <View style={styles.labelContainer}>
              <Text style={styles.labelText}>Periodic Transaction</Text>
            </View>
            <TouchableOpacity
          ...
          <View style={[styles.smallButtonContainer, { bottom: 140 }]}>
            <View style={styles.labelContainer}>
              <Text style={styles.labelText}>Singular Transaction</Text>
            </View>
            <TouchableOpacity
              style={styles.smallButton}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              <Text style={[styles.modalTitle, { fontSize: 16 }]}>Icon</Text>
              <View style={{ marginBottom: 20 }}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setIsSingularEmojiOpen(true)}
                >
                  <Text
                    style={{
                      fontSize: 25,
              ...
              <Text style={[styles.modalTitle, { fontSize: 16 }]}>Title</Text>
              <TextInput
                style={[
                  styles.textBox,
                  { width: 300, height: 50, marginBottom: 20 },
                ]}
                placeholder="Title"
              ...
              <View style={{ flexDirection: "column", marginHorizontal: 10 }}>
                <Text style={[styles.modalTitle, { fontSize: 12 }]}>
                  Category
                </Text>
                <Dropdown
              ...
              <View style={{ flexDirection: "row", alignContent: "center" }}>
                ...
                <View style={{ flexDirection: "column", marginHorizontal: 10 }}>
                  ...
                  <TextInput
                    style={[
                      styles.textBox,
                      { width: 175, height: 50, marginBottom: 20 },
                    ]}
                    placeholder="X.XX"
                    placeholderTextColor="#3c5a80"
              ...
              <TextInput
                style={[
                  styles.textBox,
                  { width: 300, height: 150, marginBottom: 20 },
                ]}
            ...
            <View style={styles.modalBody}>
              ...
              <View style={{ marginBottom: 20 }}>
                <TouchableOpacity
                  ...
                  onPress={() => setIsPeriodicEmojiOpen(true)}
                >
                  <Text
                    style={{
                      fontSize: 25,
              ...
              <View style={{ flexDirection: "row", marginBottom: 20 }}>
                <TextInput
                  style={[
                    styles.textBox,
                    { width: 60, height: 50, marginRight: 10 },
                  ]}
                  placeholderTextColor="#3c5a80"
                  value={periodicTimes}
                ...
                <TextInput
                  style={[
                    styles.textBox,
                    { width: 60, height: 50, marginLeft: 10 },
                  ]}
                  placeholderTextColor="#3c5a80"
                ...
                <Dropdown
                  style={[
                    styles.textBox,
                    { width: 120, height: 50, marginLeft: 5 },
                  ]}
                  placeholderStyle={{ color: "#3c5a80" }}
                  selectedTextStyle={{ color: "#fff" }}
                  data={periodChoices}
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalTop}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setSingularEdit(!singularEdit)}
              >
                <Text style={styles.editButtonText}>
                  {singularEdit ? "Stop Editing" : "Edit"}
                </Text>
              </TouchableOpacity>
            </View>
            <View style={styles.modalBody}>
              ...
              <View style={{ marginBottom: 20 }}>
                ...
                <TouchableOpacity
                  style={[
                    singularEdit ? styles.textBox : styles.uneditedTextBox,
                    {
                      width: 50,
                      height: 50,
                      justifyContent: "center",
            ...
            <View style={styles.modalBottom}>
              <TouchableOpacity
                style={styles.cancelButton}
                onPress={toggleSingularEditModal}
              >
                <Text style={styles.cancelButtonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.closeButton}
                onPress={handleSingularSave}
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  {/* <Text style={{fontSize: 30, color: '#fff', paddingRight: 10}}>+</Text> */}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalTop}>
              ...
              <TouchableOpacity
                style={styles.editButton}
                onPress={() => setPeriodicEdit(!periodicEdit)}
              >
                <Text style={styles.editButtonText}>
                  {periodicEdit ? "Stop Editing" : "Edit"}
                </Text>
              </TouchableOpacity>
            </View>
            <View style={styles.modalBody}>
              ...
              <View style={{ marginBottom: 20 }}>
                <TouchableOpacity
                  style={[
                    periodicEdit ? styles.textBox : styles.uneditedTextBox,
                    {
                      width: 50,
                      height: 50,
                      justifyContent: "center",
            ...
            <View style={styles.modalBottom}>
              <TouchableOpacity
                style={styles.cancelButton}
                onPress={togglePeriodicEditModal}
              >
                <Text style={styles.cancelButtonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.closeButton}
                onPress={handlePeriodicSave}
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  {/* <Text style={{fontSize: 30, color: '#fff', paddingRight: 10}}>+</Text> */}
                  <Text style={styles.closeButtonText}>Save</Text>
